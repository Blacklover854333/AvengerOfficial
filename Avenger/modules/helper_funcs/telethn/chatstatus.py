from Avenger.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Avenger import DRAGONS
...
async def avenger_is_admin(chat_id: int):
    status = False
    avenger = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
        if avenger.id == user.id:
            status = True
            break
    return status
